...
class PublicUserApiTest(TestCase):
    ...
    def test_password_too_short(self):
        ...
        res = self.client.post(CREATE_USER_URL, payload)
        self.assertEqual(res.status_code, status.HTTP_400_BAD_REQUEST)
        user_exitis = get_user_model().objects.filter(
            email=payload['email']
        ).exists()
        self.assertFalse(user_exitis)

    def test_create_token_for_user(self):
        """Test Token created fro user"""
    ...
    def test_create_token_invalid(self):
        """Login credecial is wroung"""
        create_user(email='test@test.com',password="testpass")
        payload = {'email':'test@test.com','password':'wroung'}
        res = self.client.post(TOKEN_URL,payload)
...
class PrivateUserApiTest(TestCase):
    ...
    def test_post_me_not_allowed(self):
        """Test Post Not allowed to me url"""
